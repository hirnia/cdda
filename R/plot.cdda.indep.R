...
plot.cddaindep <- function(obj = NULL, stat = NULL, ylim =  NULL){ #alpha = 0.05, 
  ...
  ## ----------------------------------------------------------------------------------------------------------
  ## obj:          a cddaindep class object
  ## stat:         a character indicating the stat of statistic to be plotted
  ##               with options c("hsic.diff", "dcor.diff", "mi.diff")
  ## Example:      cdda.indep(y ~ x * mod + z, pred = "x", data = my.data) or 
  ##               test <- y ~ x * mod + z 
  ##               cdda.indep(test, pred = "x", mod = mod, modval = "mean", data = my.data)
  ## ---------------------------------------------------------------------------------------------------------  

  # What was done (9.03):
  # x.lim: if the x-axis is numeric, that's that
  #        if the x-axis is categorical, then the x-axis is 1:length(mod.levels)
  #        then, the x-axis labels are mod.levels regardless of categorical or numeric leveling
  ...
  ########### FOR REFERENCE (from cdda.indep) ##############################
  # cdda.output[[4]] <- list("response_name" = response.name, #[[4]][1]
  #                          "pred_name"= pred, #[[4]][2]
  #                          "mod_name" = mod, #[[4]][3]
  #                          "mod_data" = data[,mod] #[[4]][4]
  #                          "mod_levels" = modval, #[[4]][5] raw for contin
  #                         ) 
  ...
  ## Plot Label and Assignment ##
    ...
    if (stat == "hsic.diff"){
      if(is.null(obj[[1]][[1]]$hsic.yx)) {
        stop("Difference tests not found, set 'diff = TRUE'.")
      }
      y.title <- "HSIC Differences (95% CI)"
    ...
    if (stat == "dcor.diff"){
      if(is.null(obj[[1]][[1]]$out.diff[ ,2])){
        stop( "dcor not found. Specify diff = TRUE." )
      }
      
      y.title <- "dCor Differences (95% CI)"
      for (i in 1:length(plot.axis)) { 
    ...
    if (stat == "mi.diff"){
      if(is.null(obj[[1]][[1]]$out.diff[ ,3])){
        stop( "mitests not found. Specify diff = TRUE." )
      }
      
      y.title <- "MI Differences (95% CI)"
