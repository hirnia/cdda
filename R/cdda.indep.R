## ------------------------------------------------------------------------------------------------------------------ ##
## File:     cdda.vardist.R
## Purpose:  Source code for CDDA independence tests.
##           Requires packages "dHSIC", "lmtest", "energy", "boot"
## Packages: boot, dHSIC, lmtest, parallel
...
cdda.indep <- function(formula = NULL, pred = NULL, mod = NULL, modval = "mean",
  ...
  ## mod:          a character indicating the variable name of the moderator which serves as an
  ##               interaction term in models.
  ## modval:       For continuous moderators, character or numeric sequence specifying how the moderator is
  ##               treated in the model. Possible characters include c("mean", "median", "JN"), and if a
  ##               numeric sequence is specified, the 'pick a point' approach will be utilized.
  ...
  ##               Must be one of the two values c("gamma", "boot").
