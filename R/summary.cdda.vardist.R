summary.cddavardist <- function(cdda.output, skew = TRUE, coskew = FALSE,
  ...
  if(cdda.output[[1]][[1]]$boot.args[1] == "bca") 
    boot_print <- paste("\n", "95% BCa bootstrap CIs (", n_resamples, " resamples)", "\n\n", sep = "") 
  if(cdda.output[[1]][[1]]$boot.args[1] == "perc") 
    boot_print <- paste("\n", "95% percentile bootstrap CIs (", n_resamples, " resamples)", "\n\n", sep = "") 
  cat("\n")
    ...
    if (skew == TRUE){
      ...
      colnames(agos) <- rep(c("Statistic", "Test Value", "p-value"), 2)
      
      for (i in seq_along(mod_names)) {
        
        agos[i, ]<- c(unlist(cdda.output$cdda_alternative[[i]]$agostino$predictor),
                      unlist(cdda.output$cdda_alternative[[i]]$agostino$predictor)
        )
    ...
    ### ----------- Boot CI Loop Setup ------------ ###
    # Empty matrix with the desired dimensions
    ...
    for (i in seq_along(mod_names)) {
      ...
      momdif.stats.kurt[i, ]<- c(cdda.output[[1]][[i]]$kurtdiff,
                                 cdda.output[[2]][[i]]$kurtdiff
                                )
     }
    ...
    colnames(ansc) <- rep(c("Statistic", "Test Value", "p-value"), 2)
    
    for (i in seq_along(mod_names)) {
      
      ansc[i, ]<- c(unlist(cdda.output$cdda_target[[i]]$anscombe$predictor),
                    unlist(cdda.output$cdda_alternative[[i]]$anscombe$predictor)
                    )
    ...
    for(i in 1:length(mod_names)){
      hoctests.skew[i, ] <- c(cdda.output[[1]][[i]]$cor12diff, cdda.output[[2]][[i]]$cor12diff) 
    ...
    #### Hyperbolic Tangent (Rtanh) # Check placement
